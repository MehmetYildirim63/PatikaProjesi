...
export class OturumService {
  ...
  // Bu Kısımda  biz kayıt işlemi için firebase veribanına  hem veri gönderiyoruz kayıt işlemi için hemde verileri çekiyoruz giriş işlemi için
  ...
  autoLogin() {
    ...
    const loadedKullanici = new Kullanici(
      kullanici.email,
      kullanici.id,
      kullanici._token,
      new Date(kullanici._tokenExpirationDate)
    );
  ...
  private handleUser(
    ...
    const kullanici = new Kullanici(email, localId, idToken, expirationDate);
